...
def main():
    ...
    with psycopg.connect(url) as c, c.cursor() as q:
        q.execute("select count(*) from public.companies")
        companies = q.fetchone()[0]
        q.execute("select count(*) from growth.company_source_data")
        source_rows = q.fetchone()[0]
        q.execute("select normalized_domain from growth.company_source_data")
        db_domains = {r[0] for r in q.fetchall()}
        ...
        if len(rows) != 1200 or len(csv_domains) != 1200 or companies != 1200 or source_rows != 1200 or csv_domains != db_domains:
            raise SystemExit(1)
