"""Platform-owned canonical contract fixtures and validation."""
...
def test_event_envelope_rejects_unsupported_version_via_registry() -> None:
    schema = load_schema("event-envelope.schema.json")
    registry = json.loads(
        (SCHEMA_DIR / "contract-version-registry.json").read_text(encoding="utf-8")
    )
    fixture = {
        "event_id": "00000000-0000-4000-8000-000000000001",
        "event_type": "tracking.observation.recorded",
    ...
    jsonschema.validate(fixture, schema)
    assert fixture["schema_version"] not in registry["supported"]["event-envelope"]
...
def test_rendered_send_treatment_rejects_legacy_simplified_shape() -> None:
    schema = load_schema("rendered-send-treatment.schema.json")
    legacy = {
