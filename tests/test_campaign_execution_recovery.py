...
def test_execute_releases_claim_when_step_completion_fails():
    class CompletionFailureDB(ExecutableCampaignDB):
        def complete_step(self, enrollment_id, claim_token, step_no, next_action_at):
            self.completed.append((enrollment_id, claim_token, step_no, next_action_at))
            return False

    db = CompletionFailureDB(claim())
    with pytest.raises(CampaignExecutionError, match="completion failed"):
        CampaignExecutionOrchestrator(db).execute(
            enrollment_id=U["enrollment_id"],
            worker_id="worker-1",
            mailbox_id=U["mailbox"],
...
def test_execute_reuses_same_idempotency_key_for_retry():
    db = ExecutableCampaignDB(claim())
    orchestrator = CampaignExecutionOrchestrator(db)
    ...
    second = orchestrator.build_send_request(
        enrollment_id=U["enrollment_id"],
        worker_id="worker-1",
        mailbox_id=U["mailbox"],
        scheduled_at=datetime(2026, 9, 5, 12, tzinfo=UTC),
    ...
    assert first["send_request"]["idempotency_key"] == second["idempotency_key"]
    assert first["send_request"]["idempotency_key"] == (
        f"campaign-send:{U['campaign_id']}:person-1:{U['step_id']}"
    )
