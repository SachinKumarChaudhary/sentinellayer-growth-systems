...
from .tinyfish_batch import DatabaseCompanySeedResolver, TinyFishBatchEnricher, TinyFishDailyEnricher, TinyFishRefreshEnricher
...
def _connection_factory() -> psycopg.Connection[object]:
    ...
    return psycopg.connect(settings.database_url, connect_timeout=settings.database_connect_timeout_seconds, options=f"-c statement_timeout={settings.database_statement_timeout_seconds}")
...
def cmd_status(_: argparse.Namespace) -> int:
    ...
    try:
        ...
        return 1
    print(json.dumps({"environment": settings.environment, "real_email_enabled": settings.real_email_enabled, "worker_id": os.environ.get("SL_WORKER_ID"), "operations": state}, indent=2, default=str))
    return 0
...
def _tinyfish_client(settings: Settings) -> TinyFishClient:
    ...
    limiter = TinyFishRateLimiter(TinyFishRateLimitPolicy(search_per_minute=settings.tinyfish_search_per_minute, search_per_hour=settings.tinyfish_search_per_hour, fetch_urls_per_minute=settings.tinyfish_fetch_urls_per_minute, fetch_urls_per_day=settings.tinyfish_fetch_urls_per_day, max_retry_attempts=settings.tinyfish_max_retry_attempts))
    return TinyFishClient(settings.tinyfish_api_key, search_url=settings.tinyfish_search_url, fetch_url=settings.tinyfish_fetch_url, timeout_seconds=settings.tinyfish_timeout_seconds, rate_limiter=limiter, max_retry_attempts=settings.tinyfish_max_retry_attempts)
...
def cmd_enrichment_research(args: argparse.Namespace) -> int:
    try:
        packet = _resolved_tinyfish_provider(_settings()).build_packet(company_id=args.company_id, domain=args.domain, merchant_name=args.merchant_name)
...
def cmd_enrichment_tinyfish_next(args: argparse.Namespace) -> int:
    try:
        settings = _settings()
        repository = EnrichmentRepository(_connection_factory)
        result = TinyFishBatchEnricher(repository=repository, provider=_resolved_tinyfish_provider(settings), seed_resolver=DatabaseCompanySeedResolver(_connection_factory)).run_next(limit=args.limit, persist=not args.dry_run)
...
def cmd_enrichment_tinyfish_daily(args: argparse.Namespace) -> int:
    try:
        ...
        repository = EnrichmentRepository(_connection_factory)
        result = TinyFishDailyEnricher(repository=repository, provider=_resolved_tinyfish_provider(settings), seed_resolver=DatabaseCompanySeedResolver(_connection_factory)).run_daily(limit=args.limit)
...
def cmd_enrichment_tinyfish_refresh(args: argparse.Namespace) -> int:
    try:
        ...
        repository = EnrichmentRepository(_connection_factory)
        result = TinyFishRefreshEnricher(repository=repository, provider=_resolved_tinyfish_provider(settings), seed_resolver=DatabaseCompanySeedResolver(_connection_factory)).run_refresh(limit=args.limit, min_age_days=args.min_age_days)
        print(json.dumps(result.__dict__, indent=2, default=str))
        return 0 if result.failed == 0 else 1
    except (ValueError, RuntimeError, psycopg.Error) as exc:
...
def cmd_enrichment_import(args: argparse.Namespace) -> int:
    try:
        ...
        result = EnrichmentRepository(_connection_factory).persist_batch(batch, provider=args.provider)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    research_cmd = enrichment_sub.add_parser("tinyfish", help="build one conservative EnrichmentPacket from TinyFish public evidence")
    ...
    batch_cmd = enrichment_sub.add_parser("tinyfish-next", help="enrich and persist the next 1-3 companies from TinyFish public evidence")
    ...
    daily_cmd = enrichment_sub.add_parser("tinyfish-daily", help="enrich up to 40 new companies using TinyFish and persist each company independently")
    ...
    refresh_cmd = enrichment_sub.add_parser("tinyfish-refresh", help="refresh up to 20 stale enriched companies, prioritizing higher-priority accounts")
