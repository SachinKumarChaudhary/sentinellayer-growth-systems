...
def normalize_signal_type(signal_type: str) -> str:
    ...
    # Prefer exact canonical names before substring aliases.
    ...
    if len(set(matches)) > 1:
        raise IntentSignalNormalizationError(
            f"ambiguous intent signal type {signal_type!r}: {sorted(set(matches))}"
        )
    return matches[0]


def normalize_signal(*, signal_type: str, signal_date: object) -> NormalizedIntentSignal:
    canonical = normalize_signal_type(signal_type)
    try:
        weight, half_life_days = {**INTENT_RULES, **COMPLIANCE_INTENT}[canonical]
    except KeyError as exc:  # pragma: no cover - protects future rule-table drift.
        raise IntentSignalNormalizationError(f"no scoring rule for {canonical!r}") from exc
...
def normalize_signals(signals: list[tuple[str, object]]) -> list[IntentSignalInput]:
    """Convert raw research labels to deterministic v2.1 scoring inputs."""
    return [normalize_signal(signal_type=signal_type, signal_date=signal_date).as_score_input() for signal_type, signal_date in signals]
